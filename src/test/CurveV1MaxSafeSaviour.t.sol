...
contract CurveV1MaxSafeSaviourTest is DSTest {
    ...
    function default_save(uint256 safe, address safeHandler) internal {
        ...
        alice.doDeposit(saviour, DSToken(address(lpToken)), safe, initTokenAmount * initTokenAmount / 10);
        assertEq(lpToken.balanceOf(address(saviour)), initTokenAmount * initTokenAmount / 10);
        assertEq(saviour.lpTokenCover(safeHandler), initTokenAmount * initTokenAmount / 10);
    ...
    function default_create_liquidatable_position_deposit_cover(uint256 liquidatableCollateralPrice)
      internal returns (address) {
        ...
        // Deposit cover
        alice.doDeposit(saviour, DSToken(address(lpToken)), safe, defaultLpTokenToDeposit);
        assertEq(lpToken.balanceOf(address(saviour)), defaultLpTokenToDeposit);
        assertEq(saviour.lpTokenCover(safeHandler), defaultLpTokenToDeposit);
    ...
    function testFail_deposit_liq_engine_not_approved() public {
        ...
        alice.doDeposit(saviour, DSToken(address(lpToken)), 1, defaultLpTokenToDeposit);
    }
    function testFail_deposit_null_lp_token_amount() public {
        ...
        alice.doDeposit(saviour, DSToken(address(lpToken)), 1, 0);
    }
    ...
    function test_deposit_twice() public {
        ...
        // Checks
        assertTrue(lpToken.balanceOf(address(saviour)) > 0 && saviour.lpTokenCover(safeHandler) > 0);
        assertEq(saviour.lpTokenCover(safeHandler), defaultLpTokenToDeposit * 2);
    ...
    function test_deposit_after_everything_withdrawn() public {
        ...
        // Deposit again
        alice.doDeposit(saviour, DSToken(address(lpToken)), safe, currentLPBalanceSaviour);
    ...
    function testFail_withdraw_unauthorized() public {
        ...
        bob.doWithdraw(saviour, safe, lpToken.balanceOf(address(saviour)), address(bob));
    }
    ...
    function test_withdraw_twice() public {
        ...
        assertTrue(lpToken.balanceOf(address(saviour)) == currentLPBalanceSaviour / 2 && saviour.lpTokenCover(safeHandler) == currentLPBalanceSaviour / 2);
    ...
    function test_getTokensForSaving_no_cover() public {
        ...
        uint sysCoins = saviour.getTokensForSaving(safeHandler, 0);

        assertEq(sysCoins, 0);
    }
    function test_getTokensForSaving_inexistent_position() public {
        uint sysCoins = saviour.getTokensForSaving(address(0x1), 100);
    ...
    function test_saveSAFE_cannot_save_safe() public {
        ...
        alice.doDeposit(saviour, DSToken(address(lpToken)), safe, defaultLpTokenToDeposit / 20);
    ...
    function test_saveSAFE_accumulate_rate() public {
        ...
        alice.doDeposit(saviour, DSToken(address(lpToken)), safe, defaultLpTokenToDeposit * 100);
