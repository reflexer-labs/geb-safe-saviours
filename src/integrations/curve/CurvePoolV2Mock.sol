...
contract CurvePoolV2Mock {
    ...
    function remove_liquidity(uint256 _amount, uint256[3] memory _min_amounts) public returns (uint256[] memory) {
        require(lpToken.transferFrom(msg.sender, address(this), _amount), "CurvePoolMock/cannot-transfer-lp-token");
    ...
    function remove_liquidity(uint256 _amount, uint256[4] memory _min_amounts) public returns (uint256[] memory) {
        require(lpToken.transferFrom(msg.sender, address(this), _amount), "CurvePoolMock/cannot-transfer-lp-token");
