...
contract SystemCoinUniswapV3SafeSaviour is SafeMath, SafeSaviourLike {
    // --- Auth ---
    ...
    **/
    modifier isAuthorized {
        require(authorizedAccounts[msg.sender] == 1, "SystemCoinUniswapV3SafeSaviour/account-not-authorized");
        _;
    }
    ...
    **/
    modifier isAllowed {
        require(
          ...
          "SystemCoinUniswapV3SafeSaviour/account-not-allowed"
        );
        _;
    }
    ...
    // The contract that can swap tokens
    SwapManagerLike                public swapManager;
    ...
    // The second token in the Uniswap pair (besides the system coin)
    ERC20Like                      public pairToken;
    ...
    constructor(
        ...
        require(coinJoin_ != address(0), "SystemCoinUniswapV3SafeSaviour/null-coin-join");
        require(collateralJoin_ != address(0), "SystemCoinUniswapV3SafeSaviour/null-collateral-join");
        require(cRatioSetter_ != address(0), "SystemCoinUniswapV3SafeSaviour/null-cratio-setter");
        require(systemCoinOrcl_ != address(0), "SystemCoinUniswapV3SafeSaviour/null-system-coin-oracle");
        require(oracleRelayer_ != address(0), "SystemCoinUniswapV3SafeSaviour/null-oracle-relayer");
        require(liquidationEngine_ != address(0), "SystemCoinUniswapV3SafeSaviour/null-liquidation-engine");
        require(safeManager_ != address(0), "SystemCoinUniswapV3SafeSaviour/null-safe-manager");
        require(saviourRegistry_ != address(0), "SystemCoinUniswapV3SafeSaviour/null-saviour-registry");
        require(liquidityManager_ != address(0), "SystemCoinUniswapV3SafeSaviour/null-liq-manager");
        require(swapManager_ != address(0), "SystemCoinUniswapV3SafeSaviour/null-swap-manager");
        require(lpToken_ != address(0), "SystemCoinUniswapV3SafeSaviour/null-lp-token");
        require(pairToken_ != address(0), "SystemCoinUniswapV3SafeSaviour/null-pair-token");
        require(minKeeperPayoutValue_ > 0, "SystemCoinUniswapV3SafeSaviour/invalid-min-payout-value");
        ...
        swapManager          = SwapManagerLike(swapManager_);
        pairToken            = ERC20Like(pairToken_);
        ...
        require(collateralJoin.contractEnabled() == 1, "SystemCoinUniswapV3SafeSaviour/join-disabled");
        require(address(collateralToken) != address(0), "SystemCoinUniswapV3SafeSaviour/null-col-token");
        require(address(safeEngine) != address(0), "SystemCoinUniswapV3SafeSaviour/null-safe-engine");
        require(address(systemCoin) != address(0), "SystemCoinUniswapV3SafeSaviour/null-sys-coin");
        ...
        emit ModifyParameters("swapManager", swapManager_);
    }
    ...
    function modifyParameters(bytes32 parameter, uint256 val) external isAuthorized {
        if (parameter == "minKeeperPayoutValue") {
            require(val > 0, "SystemCoinUniswapV3SafeSaviour/null-min-payout");
            minKeeperPayoutValue = val;
        }
        else if (parameter == "restrictUsage") {
            require(val <= 1, "SystemCoinUniswapV3SafeSaviour/invalid-restriction");
            restrictUsage = val;
        }
        else revert("SystemCoinUniswapV3SafeSaviour/modify-unrecognized-param");
    ...
    function modifyParameters(bytes32 parameter, address data) external isAuthorized {
        require(data != address(0), "SystemCoinUniswapV3SafeSaviour/null-data");
    ...
    */
    function deposit(uint256 safeID, uint256 lpTokenAmount) external isAllowed() liquidationEngineApproved(address(this)) nonReentrant {
        require(lpTokenAmount > 0, "SystemCoinUniswapV3SafeSaviour/null-lp-amount");

        // Check that the SAFE exists inside GebSafeManager
        address safeHandler = safeManager.safes(safeID);
        require(safeHandler != address(0), "SystemCoinUniswapV3SafeSaviour/null-handler");

        // Check that the SAFE has debt
        (, uint256 safeDebt) =
          SAFEEngineLike(collateralJoin.safeEngine()).safes(collateralJoin.collateralType(), safeHandler);
        require(safeDebt > 0, "SystemCoinUniswapV3SafeSaviour/safe-does-not-have-debt");
        ...
        require(lpToken.transferFrom(msg.sender, address(this), lpTokenAmount), "SystemCoinUniswapV3SafeSaviour/could-not-transfer-lp");
    ...
    */
    function withdraw(uint256 safeID, uint256 lpTokenAmount, address dst) external controlsSAFE(msg.sender, safeID) nonReentrant {
        ...
        address safeHandler = safeManager.safes(safeID);
        require(lpTokenCover[safeHandler] >= lpTokenAmount, "SystemCoinUniswapV3SafeSaviour/not-enough-to-withdraw");
    ...
    */
    function getReserves(uint256 safeID, address dst) external controlsSAFE(msg.sender, safeID) nonReentrant {
        ...
        require(either(systemCoins > 0, collateralCoins > 0), "SystemCoinUniswapV3SafeSaviour/no-reserves");
    ...
    */
    function saveSAFE(address keeper, bytes32 collateralType, address safeHandler) override external returns (bool, uint256, uint256) {
        require(address(liquidationEngine) == msg.sender, "SystemCoinUniswapV3SafeSaviour/caller-not-liquidation-engine");
        require(keeper != address(0), "SystemCoinUniswapV3SafeSaviour/null-keeper-address");

        if (both(both(collateralType == "", safeHandler == address(0)), keeper == address(liquidationEngine))) {
            return (true, uint(-1), uint(-1));
        }

        require(collateralType == collateralJoin.collateralType(), "SystemCoinUniswapV3SafeSaviour/invalid-collateral-type");

        // Check that the SAFE has a non null amount of LP tokens covering it
        require(lpTokenCover[safeHandler] > 0, "SystemCoinUniswapV3SafeSaviour/null-cover");
        ...
        collateralCoinBalance = sub(collateralToken.balanceOf(address(this)), collateralCoinBalance);

        // Get the amount of tokens used to top up the SAFE
        (uint256 safeDebtRepaid, uint256 safeCollateralAdded) =
          getTokensForSaving(safeHandler, oracleRelayer.redemptionPrice(), sysCoinBalance, collateralCoinBalance);

        // There must be tokens used to save the SAVE
        require(either(safeDebtRepaid > 0, safeCollateralAdded > 0), "SystemCoinUniswapV3SafeSaviour/cannot-save-safe");
        ...
        require(either(keeperSysCoins > 0, keeperCollateralCoins > 0), "SystemCoinUniswapV3SafeSaviour/cannot-pay-keeper");
        ...
        collateralCoinBalance = sub(
          collateralCoinBalance, add(safeCollateralAdded, keeperCollateralCoins)
        );
    ...
    */
    function getTokensForSaving(address safeHandler, uint256 redemptionPrice, uint256 sysCoinsFromLP, uint256 collateralFromLP)
      public view returns (uint256, uint256) {
        if (either(redemptionPrice == 0, both(sysCoinsFromLP == 0, collateralFromLP == 0))) {
            return (0, 0);
        }
