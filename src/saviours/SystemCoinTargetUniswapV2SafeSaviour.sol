...
contract SystemCoinTargetUniswapV2SafeSaviour is SafeMath, SafeSaviourLike {
    // --- Auth ---
    ...
    **/
    modifier isAuthorized {
        require(authorizedAccounts[msg.sender] == 1, "SystemCoinTargetUniswapV2SafeSaviour/account-not-authorized");
        _;
    }
    ...
    **/
    modifier isAllowed {
        require(
          ...
          "SystemCoinTargetUniswapV2SafeSaviour/account-not-allowed"
        );
        _;
    }
    ...
    constructor(
        ...
        require(coinJoin_ != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-coin-join");
        require(collateralJoin_ != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-collateral-join");
        require(cRatioSetter_ != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-cratio-setter");
        require(systemCoinOrcl_ != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-system-coin-oracle");
        require(oracleRelayer_ != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-oracle-relayer");
        require(liquidationEngine_ != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-liquidation-engine");
        require(safeManager_ != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-safe-manager");
        require(saviourRegistry_ != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-saviour-registry");
        require(liquidityManager_ != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-liq-manager");
        require(swapManager_ != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-swap-manager");
        require(lpToken_ != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-lp-token");
        require(pairToken_ != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-pair-token");
        require(minKeeperPayoutValue_ > 0, "SystemCoinTargetUniswapV2SafeSaviour/invalid-min-payout-value");
        ...
        require(collateralJoin.contractEnabled() == 1, "SystemCoinTargetUniswapV2SafeSaviour/join-disabled");
        require(address(collateralToken) != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-col-token");
        require(address(safeEngine) != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-safe-engine");
        require(address(systemCoin) != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-sys-coin");
    ...
    function modifyParameters(bytes32 parameter, uint256 val) external isAuthorized {
        if (parameter == "minKeeperPayoutValue") {
            require(val > 0, "SystemCoinTargetUniswapV2SafeSaviour/null-min-payout");
            minKeeperPayoutValue = val;
        }
        else if (parameter == "restrictUsage") {
            require(val <= 1, "SystemCoinTargetUniswapV2SafeSaviour/invalid-restriction");
            restrictUsage = val;
        }
        else revert("SystemCoinTargetUniswapV2SafeSaviour/modify-unrecognized-param");
    ...
    function modifyParameters(bytes32 parameter, address data) external isAuthorized {
        require(data != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-data");
    ...
    */
    function deposit(uint256 safeID, uint256 lpTokenAmount) external isAllowed() liquidationEngineApproved(address(this)) nonReentrant {
        require(lpTokenAmount > 0, "SystemCoinTargetUniswapV2SafeSaviour/null-lp-amount");

        // Check that the SAFE exists inside GebSafeManager
        address safeHandler = safeManager.safes(safeID);
        require(safeHandler != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-handler");

        // Check that the SAFE has debt
        (, uint256 safeDebt) =
          SAFEEngineLike(collateralJoin.safeEngine()).safes(collateralJoin.collateralType(), safeHandler);
        require(safeDebt > 0, "SystemCoinTargetUniswapV2SafeSaviour/safe-does-not-have-debt");
        ...
        require(lpToken.transferFrom(msg.sender, address(this), lpTokenAmount), "SystemCoinTargetUniswapV2SafeSaviour/could-not-transfer-lp");
    ...
    */
    function withdraw(uint256 safeID, uint256 lpTokenAmount, address dst) external controlsSAFE(msg.sender, safeID) nonReentrant {
        ...
        address safeHandler = safeManager.safes(safeID);
        require(lpTokenCover[safeHandler] >= lpTokenAmount, "SystemCoinTargetUniswapV2SafeSaviour/not-enough-to-withdraw");
    ...
    */
    function getReserves(uint256 safeID, address dst) external controlsSAFE(msg.sender, safeID) nonReentrant {
        ...
        require(either(systemCoins > 0, collateralCoins > 0), "SystemCoinTargetUniswapV2SafeSaviour/no-reserves");
    ...
    */
    function saveSAFE(address keeper, bytes32 collateralType, address safeHandler) override external returns (bool, uint256, uint256) {
        require(address(liquidationEngine) == msg.sender, "SystemCoinTargetUniswapV2SafeSaviour/caller-not-liquidation-engine");
        require(keeper != address(0), "SystemCoinTargetUniswapV2SafeSaviour/null-keeper-address");
        ...
        require(collateralType == collateralJoin.collateralType(), "SystemCoinTargetUniswapV2SafeSaviour/invalid-collateral-type");

        // Check that the SAFE has a non null amount of LP tokens covering it
        require(lpTokenCover[safeHandler] > 0, "SystemCoinTargetUniswapV2SafeSaviour/null-cover");
        ...
        require(either(safeDebtRepaid > 0, safeCollateralAdded > 0), "SystemCoinTargetUniswapV2SafeSaviour/cannot-save-safe");
        ...
        require(either(keeperSysCoins > 0, keeperCollateralCoins > 0), "SystemCoinTargetUniswapV2SafeSaviour/cannot-pay-keeper");
        ...
        require(
          ...
          "SystemCoinTargetUniswapV2SafeSaviour/faulty-remove-liquidity"
        );
    ...
    */
    function getLPUnderlying(address safeHandler) public view returns (uint256, uint256) {
        ...
        (uint256 sysCoinsFromLP, uint256 pairTokenFromLP) = (isSystemCoinToken0) ?
        ...
        uint256 collateralFromLP = swapManager.getAmountOut(address(pairToken), address(collateralToken), pairTokenFromLP);
